...
const GoTrackSlots = ({ route }) => {
  ...
  console.log("onDemandProduct", onDemandProduct?.productProvider?.id);

  console.log("inCartOrder", inCartOrder?.services[0]);
  ...
  const getServiceProviderSlots = () => {
    setLoading(true);
    GetServiceProviderSlots({
      serviceProvider: onDemandProduct?.productProvider,
      service: onDemandProduct?.product,
    }).then((response) => {
