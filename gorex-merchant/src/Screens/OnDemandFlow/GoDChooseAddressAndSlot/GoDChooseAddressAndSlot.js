...
const GoDChooseAddressAndSlot = ({ route }) => {
  const { userProfile, onDemandOrder, setOnDemandOrder, setSelectedBranch } =
  ...
  const [selectedServiceProvider, setSelectedServiceProvider] = useState(
    onDemandOrder && onDemandOrder.serviceProvider
  ...
  useEffect(() => {
    ...
    analytics().logScreenView({
      screen_name: "GonD_addresses_and_slots_screen",
    });
  }, []);
  ...
  const onPressNextButton = async () => {
    ...
    if (selectedServiceProvider && !isObjEmpty(modifiedOrder)) {
      ...
      navigation.navigate("Slots", {
        isOnDemand: true,
        COD: selectedServiceProvider?.is_cod_active,
      });
    } else {
      await setOnDemandOrder({
        address: selectedAddress,
        ...
        notes: onDemandOrder?.notes,
        service: onDemandOrder?.service,
        serviceProvider: !isObjEmpty(onDemandOrder?.serviceProvider)
          ? onDemandOrder?.serviceProvider
  ...
  return (
    <View style={styles.screen}>
      <BackHeader
        ...
        leftPress={() =>
          navigation.navigate("GoDChooseService", { check: false })
        }
      />
      ...
      <View style={{ flex: 0.15, justifyContent: "flex-end" }}>
        <Footer
          title={t("common.next")}
          disabled={isObjEmpty(selectedServiceProvider) || !selectedAddress}
